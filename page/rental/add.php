...
if (isset($_POST['submit'])) {
    ...
    echo "
        <script>
            document.location.href = '?page=rental&pesan=Data Berhasil Ditambahkan!';
        </script>
        ";
}
?>
<form action="?page=rental&action=add" method="POST">
    <div class="row">
        <div class="col-sm-6">
            <div class="mb-3">
                <label for="no_trx_gian_sonia" class="form-label">No Transaksi</label>
                <input type="text" name="no_trx_gian_sonia" id="no_trx_gian_sonia" class="form-control" placeholder="Masukan NIK" value="<?= $trx; ?>" readonly required>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" name="nik_ktp_gian_sonia" id="nik_ktp_gian_sonia" required>
                    ...
                    <?php foreach ($pelanggans as $pelanggan) : ?>
                        <option value="<?= $pelanggan['nik_ktp_gian_sonia']; ?>"><?= $pelanggan['nik_ktp_gian_sonia'] . ' - ' . $pelanggan['nama_gian_sonia']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" name="no_plat_gian_sonia" id="no_plat_gian_sonia" required>
                    ...
                    <?php foreach ($mobils as $mobil) : ?>
                        <option value="<?= $mobil['no_plat_gian_sonia']; ?>"><?= $mobil['no_plat_gian_sonia'] . ' - ' . $mobil['nama_mobil_gian_sonia']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        <div class="col-sm-6">
            <div class="mb-3">
                ...
                <input type="date" name="tanggal_rental_gian_sonia" id="tanggal_rental_gian_sonia" class="form-control" required>
            </div>
            <div class=" mb-3">
                <label for="jam_rental_gian_sonia" class="form-label">Jam Ambil</label>
                <input type="time" name="jam_rental_gian_sonia" id="jam_rental_gian_sonia" class="form-control" required>
            </div>
            <div class=" mb-3">
                <label for="lama_gian_sonia" class="form-label">Lama Rental</label>
                <input type="number" name="lama_gian_sonia" id="lama_gian_sonia" class="form-control" required>
            </div>
            <div class=" mb-3">
                <label for="harga_gian_sonia" class="form-label">Harga Rental</label>
                <input type="text" name="harga_gian_sonia" id="harga_gian_sonia" class="form-control money" required>
            </div>
            <div class=" mb-3">
                <label for="total_bayar_gian_sonia" class="form-label">Total Bayar</label>
                <input type="text" name="total_bayar_gian_sonia" id="total_bayar_gian_sonia" readonly class="form-control money" required>
            </div>
            <div class=" mb-3">
